"""CLI integration tests against PostgreSQL.
...
@skip_integration
class TestCLIPostgreSQL:
    """CLI integration tests using PostgreSQL backend."""
    ...
    def test_full_workflow_create_metadata(self):
        ...
        result = self.runner.invoke(cli, [
            "connection", "create",
            "--name", f"cli_pg_conn_{self.unique}",
            "--type", "postgresql",
            "--host", "localhost",
            "--port", "5433",
            "--environment", "dev",
        ...
        result = self.runner.invoke(cli, [
            "process", "create",
            "--name", f"cli_pg_proc_{self.unique}",
            "--order", "1",
        ])
        assert result.exit_code == 0
        ...
        assert result.exit_code == 0
        assert f"cli_pg_proc_{self.unique}" in result.output
